...
async function GenerateStoryboard(prompt) {
  ...
  while (retries > 0) {
    try {
      let response = await callGPT(storyboard_prompt, [], prompt.trim());
      ...
      if (errors.length > 0) {
        ...
        response = await callGPT(storyboard_prompt, history, correctingPrompt);
        parsedObject = JSON.parse(response);
        errors = validlateMainPrompt(parsedObject)
        
        if (errors.length > 0) {
          const errorMsg = "Failed JSON validation upon re-request: " + errors.join(',');
...
async function callGPT(seed, history, prompt) {
  payload = [];
